...
public class IdleConsumptionSubTLV extends PCEPSubTLV {
	...
	public IdleConsumptionSubTLV(){
		this.setSubTLVType(PCEPSubTLVTypes.PCEP_SUBTLV_TYPE_IDLE_CONSUMPTION);
		
	}
	
	public IdleConsumptionSubTLV(byte[] bytes, int offset){
		super(bytes,offset);
		decode();
	}
	...
	public void encode() {
		...
		this.encodeHeader();
		System.arraycopy(idle, 0, this.subtlv_bytes, 4, 4);
	}

	
	public void decode() {
		System.arraycopy(this.subtlv_bytes, 4, this.idle, 0, 4);
	}


	public void setIdleConsumption(byte[] idle) {
		this.idle = idle;
	}
	
	public byte[] getIdleConsumption() {
		return idle;
	}
