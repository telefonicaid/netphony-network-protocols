...
import tid.rsvp.constructs.te.SEFlowDescriptorTE;
import tid.rsvp.messages.RSVPMessageTypes;
import tid.rsvp.messages.RSVPResvMessage;
...
public class RSVPTEResvMessage extends RSVPResvMessage {
	...
	public RSVPTEResvMessage(){
		...
		log.finest("RSVP-TE Resv Message Created");
	}
	...
	public void encode() throws RSVPProtocolViolationException {
		...
		log.finest("Starting RSVP-TE Resv Message encode");
		...
		log.finest("RSVP-TE Resv Message encoding accomplished");
	}
		...
		while(offset < length){		// Mientras quede mensaje
			...
			if(classNum == 1){
				...
				if(cType == 1){
					...
					log.severe("Error Deconding Time Values");
					// Fallo en cType
					throw new RSVPProtocolViolationException();
				...
				if(style != null){	// Style debe haberse decodificado porque en caso
					...
					log.severe("Malformed RSVP-TE Resv Message, Style Object not Found");
					throw new RSVPProtocolViolationException();
				}
			}	
			else{
				// Fallo en classNum
				log.severe("Malformed RSVP-TE Resv Message, Object classNum incorrect");
				throw new RSVPProtocolViolationException();
			}
		}
	}
